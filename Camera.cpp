...
{
	/*glm::mat4 res= this->lookAt(eye, at, up);
	res = res * instance;
	return res;*/
	return this->lookAt(eye, at, up);
}
void Camera::update_dirction(float num1, float num2, float num3)
{
	_x = num1;
	_y = num2;
	_z = num2;
...
void  Camera::sroll(double x, double y)
{
	if (y > 0)
	{
		radius -= 0.4;
...
void Camera::mousePos(double x, double y)
{
	if (firstmouse)
	{
		lastX = x;
	...
	rotateAngle += 1* (x - float(600) / 2.0) / 600;
	rotateAngle = glm::mod(rotateAngle + 180.0f, 360.0f) - 180.0f;
	upAngle += -1 * (y - float(600) / 2.0) / 600;
	upAngle = glm::clamp(upAngle, -89.0f, 89.0f);*/


	float xoffset = x - lastX;
	float yoffset = y - lastY;
	//std::cout << xoffset << "   " << yoffset <<"<---->"<<x<<"   "<<y <<"<----->"<<lastX<<"   "<<lastY << std::endl;
	lastX = x;
	lastY = y;
	float sensitivety = 0.25f;
	xoffset *= sensitivety;
	yoffset *= sensitivety;
	upAngle += yoffset;
	rotateAngle -= xoffset;
...
void Camera::keyboard(int key, int action, int mode)
{
	...
	{
		instance = glm::translate(instance, glm::vec3(0.0, 0.0, 0.4));
	}
	else if (key == GLFW_KEY_UP && mode == GLFW_MOD_SHIFT)
	{
	...
	else if (key == GLFW_KEY_DOWN && mode == GLFW_MOD_SHIFT)
	{
	...
	else if (key == GLFW_KEY_LEFT && mode == GLFW_MOD_SHIFT)
	{
	...
	else if (key == GLFW_KEY_RIGHT && mode == GLFW_MOD_SHIFT)
	{
