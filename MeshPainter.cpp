...
void MeshPainter::bindObjectAndData(TriMesh *mesh, openGLObject &object, const std::string &texture_image, const std::string &vshader, const std::string &fshader){
    ...
    glVertexAttribPointer(object.tLocation, 2,
        GL_FLOAT, GL_FALSE, 0,
        BUFFER_OFFSET((points.size() + normals.size() + colors.size()) * sizeof(glm::vec3), textures.size() * sizeof(glm::vec2)));
...
void MeshPainter::drawMesh(TriMesh* mesh, openGLObject &object, Light *light, Camera* camera){
    ...
    glm::mat4 modelMatrix1 = mesh->getModelMatrix();
    glm::mat4 c = light->getShadowProjectionMatrix();

    glm::mat4 temp = c * modelMatrix1;
    
    glUniformMatrix4fv(object.modelLocation, 1, GL_FALSE, &temp[0][0]);
...
//用于绘制机器人的身体各个部分
void MeshPainter::drawMesh3(TriMesh* mesh, openGLObject& object, Light* light, Camera* camera,glm::mat4 instance) {
...
//用于绘制机器人的各个部分
void MeshPainter::drawRobot(Light* light, Camera* camera,glm::mat4 instance)
{
    for (int i = 0; i < meshes.size(); i++)
    {
        drawMesh3(meshes[i], opengl_objects[i], light, camera, instance);
    }
}
